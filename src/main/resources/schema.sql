...
CREATE TABLE "LIKES"                                -- создание таблицы лайков (userId - filmId)
(
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    filmId bigint,
    userId bigint,
    CONSTRAINT unique_userId_filmId UNIQUE ( filmId, userId),
    CONSTRAINT filmId_likes_fk FOREIGN KEY (filmId) REFERENCES FILMS (id) ON DELETE CASCADE,
    CONSTRAINT userId_likes_fk FOREIGN KEY (userId) REFERENCES USERS (id) ON DELETE CASCADE
);


CREATE TABLE "FILMS_GENRES"                             -- создание вспомогательной таблицы жанров (filmId - genre_id)
(
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    filmId bigint,
    genre_id bigint,
    CONSTRAINT f_g_key UNIQUE (filmId, genre_id),
    CONSTRAINT filmId_f_g_fk FOREIGN KEY (filmId) REFERENCES FILMS (id) ON DELETE CASCADE,
    CONSTRAINT genre_id_f_g_fk FOREIGN KEY (genre_id) REFERENCES GENRES (id) ON DELETE CASCADE
);


CREATE TABLE "FRIENDSHIP"                             -- создание вспомогательной таблицы дружбы (userId - friend_id)
(
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    userId bigint,
    friend_id bigint,
    CONSTRAINT unique_friendship UNIQUE (userId, friend_id),
    CONSTRAINT userId_frnd_fk FOREIGN KEY (userId) REFERENCES USERS (id) ON DELETE CASCADE,
